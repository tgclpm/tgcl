...
import { GLOBAL_OBJ, escapeStringForRegex, logger, arrayify, stringMatchesSomePattern, dropUndefinedKeys } from '@sentry/utils';
...
function constructWebpackConfigFunction(
  ...
  return function newWebpackFunction(
    ...
    if (isServer) {
      ...
      const nftPlugin = _optionalChain([newConfig, 'access', _ => _.plugins, 'optionalAccess', _2 => _2.find, 'call', _3 => _3((plugin) => {
    ...
    // fairly massively.
    if (!isServer && _optionalChain([userSentryOptions, 'optionalAccess', _4 => _4.transpileClientSDK])) {
      // Find all loaders which apply transpilation to user code
      const transpilationRules = findTranspilationRules(_optionalChain([newConfig, 'access', _5 => _5.module, 'optionalAccess', _6 => _6.rules]), projectDir);
...
function isMatchingRule(rule, projectDir) {
  ...
  if (!useEntries.some(entry => _optionalChain([entry, 'optionalAccess', _7 => _7.loader]) && new RegExp('next.*(babel|swc).*loader').test(entry.loader))) {
    return false;
  }
...
function setUpModuleRules(newConfig) {
  newConfig.module = {
    ...newConfig.module,
    rules: [...(_optionalChain([newConfig, 'access', _8 => _8.module, 'optionalAccess', _9 => _9.rules]) || [])],
  };
...
function addValueInjectionLoader(
  ...
  const serverValues = {
    ...
    // characters)
    __rewriteFramesDistDir__: _optionalChain([userNextConfig, 'access', _10 => _10.distDir, 'optionalAccess', _11 => _11.replace, 'call', _12 => _12(/\\/g, '\\\\')]) || '.next',
  };
