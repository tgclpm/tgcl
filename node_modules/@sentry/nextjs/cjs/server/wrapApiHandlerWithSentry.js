...
const nextLogger = require('./utils/nextLogger.js');
...
function wrapApiHandlerWithSentry(
  ...
  if (maybeWrappedHandler.name === 'sentryWrappedHandler') {
    const [_sentryNextjs_, _autoWrapOption_, _withSentry_, _route_] = [
      ...
      'withSentry',
      parameterizedRoute,
    ].map(phrase => nextLogger.formatAsCode(phrase));

    nextLogger.nextLogger.info(
    ...
    const boundHandler = local.bind(async () => {
      ...
      if (currentScope) {
        ...
        if (tracing.hasTracingEnabled(options) && _optionalChain([options, 'optionalAccess', _5 => _5.instrumenter]) === 'sentry') {
          ...
          if (platformSupportsStreaming.platformSupportsStreaming() && !origHandler.__sentry_test_doesnt_support_streaming__) {
